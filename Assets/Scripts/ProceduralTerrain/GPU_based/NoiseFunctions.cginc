...
float simplex(float2 input)
{
    ...
    // sort of shearing along the x = y line, hence why we add both x and y
    int2 o = int2(
    ...
    float2 p[3];
    // distance from origin, in unskewed space
    p[0] = float2(
        input.x - o.x + unskew,
        input.y - o.y + unskew
    );
    ...
    // distance from middle corner in unskewed space
    p[1] = float2(
        p[0].x - offs.x + g2,
        p[0].y - offs.y + g2
    );
    
    // distance from end corner in unskewed space
    p[2] = float2(
        p[0].x - 1.0 + 2.0 * g2,
        p[0].y - 1.0 + 2.0 * g2
    );
